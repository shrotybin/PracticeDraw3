...
public class Practice13GetTextBoundsView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int width=(rect.bottom+rect.top)/2;
        canvas.drawText(text1, 100, middle-width, paint2);

        paint2.getTextBounds(text2,0,text2.length(),rect);
        width=(rect.bottom+rect.top)/2;
        canvas.drawText(text2, 200, middle-width, paint2);

        paint2.getTextBounds(text3,0,text3.length(),rect);
        width=(rect.bottom+rect.top)/2;
        canvas.drawText(text3, 300, middle-width, paint2);

        paint2.getTextBounds(text4,0,text4.length(),rect);
        width=(rect.bottom+rect.top)/2;
        canvas.drawText(text4, 400, middle-width, paint2);

        paint2.getTextBounds(text5,0,text5.length(),rect);
        width=(rect.bottom+rect.top)/2;
        canvas.drawText(text5, 500, middle-width, paint2);

        paint2.getTextBounds(text6,0,text6.length(),rect);
        width=(rect.bottom+rect.top)/2;
        canvas.drawText(text6, 600, middle-width, paint2);
    }
}